...
const App = () => {
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-[#050505] text-slate-100 font-sans">
      ...
      <nav className="absolute top-2 left-2 right-2 md:top-4 md:left-4 md:right-4 z-[100] flex flex-col md:flex-row gap-2 pointer-events-none">
        
        <div className="flex items-center justify-between w-full gap-2">
            {/* Logo Section */}
            <div className="flex items-center gap-2 pointer-events-auto bg-black/60 backdrop-blur-xl border border-white/10 px-3 py-2 rounded-2xl">
              <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-1.5 hover:bg-white/10 rounded-xl">
                {isSidebarOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
              </button>
              <div className="flex items-center gap-1.5 pr-1">
                <Cloud className="w-5 h-5 text-blue-400" />
                <span className="text-xs font-black tracking-widest uppercase block">Vhiteskies</span>
              </div>
            </div>

            {/* View Toggles */}
            <div className="flex items-center gap-2 pointer-events-auto">
              <button onClick={() => setIsSatellite(!isSatellite)} className={`p-2.5 rounded-xl backdrop-blur-xl border transition-all ${isSatellite ? 'bg-blue-500 border-blue-400' : 'bg-black/60 border-white/10'}`}>
                <Globe className="w-4 h-4" />
              </button>
              <button onClick={() => setIsFullMap(!isFullMap)} className={`p-2.5 rounded-xl backdrop-blur-xl border transition-all ${isFullMap ? 'bg-emerald-500 border-emerald-400' : 'bg-black/60 border-white/10'}`}>
        ...
        <div className="relative flex-1 pointer-events-auto w-full md:max-w-lg">
      ...
      {/* Layer Controls - Shifted for mobile accessibility */}
      <div className="absolute right-2 top-32 md:right-4 md:top-24 z-40 flex flex-col gap-2 pointer-events-auto">
        {[{ id: 'precipitation_new', icon: Waves }, { id: 'wind_new', icon: WindIcon }, { id: 'pressure_new', icon: Gauge }, { id: 'temp_new', icon: Thermometer }].map(layer => (
          <button key={layer.id} onClick={() => updateMapLayer(layer.id)} className={`p-3 md:p-4 rounded-xl md:rounded-2xl backdrop-blur-xl border transition-all ${activeLayer === layer.id ? 'bg-blue-500 border-blue-400' : 'bg-black/60 border-white/10'}`}>
            <layer.icon className="w-4 h-4 md:w-5 md:h-5" />
          </button>
        ))}
      </div>

      {/* Bottom Dashboard - Stacks on Mobile */}
      <AnimatePresence>
        {!isFullMap && (
          <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} 
            className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6 z-30 pointer-events-none flex flex-col md:flex-row items-stretch gap-3">
            
            {/* 24h Timeline (Primary on Mobile) */}
            {forecast && (
              <div className="flex-1 h-[180px] md:h-[320px] bg-black/60 backdrop-blur-3xl border border-white/10 rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 pointer-events-auto flex flex-col order-1">
            ...
            {/* Navigation/Weather Details (Secondary on Mobile - Appears BELOW Progression) */}
            <AnimatePresence>
              {isSidebarOpen && weather && (
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} 
                  className="w-full md:w-[400px] h-auto md:h-[320px] bg-black/80 backdrop-blur-3xl border border-white/10 rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-5 pointer-events-auto flex flex-col justify-between gap-4 order-2">
                  <div className="flex justify-between items-center">
                    <h2 className="text-lg md:text-xl font-black">{weather.name}</h2>
                    <div className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${getAQILevel(airQuality?.main?.aqi).bg} ${getAQILevel(airQuality?.main?.aqi).color}`}>
                  ...
                  <div className={`p-2 md:p-3 rounded-xl border flex justify-around items-center bg-opacity-10 ${getAQILevel(airQuality?.main?.aqi).bg} ${getAQILevel(airQuality?.main?.aqi).borderColor}`}>
                    <div className="text-center"><p className="text-[7px] font-black opacity-60">PM2.5</p><p className="text-[10px] md:text-xs font-black">{airQuality?.components.pm2_5.toFixed(1)}</p></div>
                    <div className="text-center"><p className="text-[7px] font-black opacity-60">SO2</p><p className="text-[10px] md:text-xs font-black">{airQuality?.components.so2.toFixed(1)}</p></div>
                    <div className="text-center"><p className="text-[7px] font-black opacity-60">NO2</p><p className="text-[10px] md:text-xs font-black">{airQuality?.components.no2.toFixed(1)}</p></div>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
      ...
      <style>{`
        ...
        .leaflet-tile { filter: brightness(0.8) contrast(1.2) saturate(0.8) !important; }
        @media (max-width: 768px) {
            .leaflet-tile { filter: brightness(0.9) contrast(1.1) !important; }
        }
      `}</style>
    </div>
  );
